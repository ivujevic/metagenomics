...
class SAMLine:
    ...
    def VerboseFormatLine(self):
        ...
        line = 'qname = %s\tactual_ref_pos = %d\tmapped_pos_with_shift = %d\tmin_distance = %d\tmapq = %d\tactual_ref_reverse = %d\treverse = %d\tis_correct_ref_and_orient = %d\tis_header_deformed = %d\tflag = %d\trname = %s\tpos = %d\tis_duplicate = %d\tis_filtered_out = %d\tis_best_of_duplicates = %d\tsecondary = %s\tpaired = %s\t' % (
            self.qname, self.actual_ref_pos, self.mapped_pos_with_shift, self.min_distance, self.mapq,
            self.actual_ref_reverse, self.IsReverse(), int(self.is_correct_ref_and_orient), self.is_header_deformed,
            self.flag, self.rname, self.pos, int(self.is_duplicate), int(self.is_filtered_out),
            int(self.is_best_of_duplicates), str(self.IsSecondary()), self.IsPaired());
    ...
    def ParseLine(self, line, sam_basename=''):
        ...
        while i < len(split_line):
            ...
            if (len(split_optional) < 2):
                i += 1;
                continue;
            self.optional[split_optional[0].strip()] = split_optional[
                -1].strip();  # Example of an optional parameter: AS:i:717 .
            i += 1;
        ...
        if ('X3' in self.optional):
            ...
            self.optional_graphmap['supp'] = float(m.group(1)) if (m and m.groups) else 1.0;
            m = re.search(r'_AS\[(.*?)\]', params);
            self.optional_graphmap['AS'] = float(m.group(1)) if (m and m.groups) else 1.0;
            m = re.search(r'_AS_std\[(.*?)\]', params);
            self.optional_graphmap['AS_std'] = float(m.group(1)) if (m and m.groups) else 1.0;
            m = re.search(r'_AS_cov_bases\[(.*?)\]', params);
            self.optional_graphmap['AS_cov_bases'] = float(m.group(1)) if (m and m.groups) else 1.0;
            m = re.search(r'_AS_read_len\[(.*?)\]', params);
            self.optional_graphmap['AS_read_len'] = float(m.group(1)) if (m and m.groups) else 1.0;
            m = re.search(r'_AS_query_len\[(.*?)\]', params);
            self.optional_graphmap['AS_query_len'] = float(m.group(1)) if (m and m.groups) else 1.0;
            m = re.search(r'_num_kmers=(.*?)_', params);
            self.optional_graphmap['num_kmers'] = float(m.group(1)) if (m and m.groups) else 1.0;
            m = re.search(r'_cov_bases=(.*?)_', params);
            self.optional_graphmap['cov_bases'] = float(m.group(1)) if (m and m.groups) else 1.0;
            m = re.search(r'lcs_length=(.*?)_', params);
            self.optional_graphmap['lcs_length'] = float(m.group(1)) if (m and m.groups) else 1.0;
            m = re.search(r'_match_rate=(.*?)_', params);
            self.optional_graphmap['match_rate'] = float(m.group(1)) if (m and m.groups) else 1.0;
            m = re.search(r'_mismatch_rate=(.*?)\t', params);
            self.optional_graphmap['mismatch_rate'] = float(m.group(1)) if (m and m.groups) else 0.0;
            m = re.search(r'_num_eq_ops=(.*?)_', params);
            self.optional_graphmap['num_eq_ops'] = float(m.group(1)) if (m and m.groups) else 0.0;
            m = re.search(r'_num_x_ops=(.*?)_', params);
            self.optional_graphmap['num_x_ops'] = float(m.group(1)) if (m and m.groups) else 0.0;
            m = re.search(r'_num_i_ops=(.*?)_', params);
            self.optional_graphmap['num_i_ops'] = float(m.group(1)) if (m and m.groups) else 0.0;
            m = re.search(r'_num_d_ops=(.*?)_', params);
            self.optional_graphmap['num_d_ops'] = float(m.group(1)) if (m and m.groups) else 0.0;
    ...
    def FormatAccuracy(self):
        ...
        line += 'len = %d\t' % (len(self.seq) - (self.clip_count_front if (self.clip_op_front == 'S') else 0) - (
    ...
    def CalcCigarStartingPositions(self, separate_matches_in_individual_bases=False, switch_ins_and_dels=False):
        ...
        while (i < len(cigar_list)):
            ...
            if (separate_matches_in_individual_bases == False):
                ...
                elif ((switch_ins_and_dels == False and cigar_op == 'D') or (
                        switch_ins_and_dels == True and cigar_op == 'I')):
                    pos_on_reference += cigar_count;
                elif ((switch_ins_and_dels == False and cigar_op == 'I') or (
                        switch_ins_and_dels == True and cigar_op == 'D')):
                    pos_on_read += cigar_count;
        ...
        if (qname in ret):
            ...
            for existing_sam in current_hash:
                if (sam_line.IsSecondary() == True) or (
                        sam_line.IsSecondary() == existing_sam.IsSecondary() and sam_line.IsReverse() == existing_sam.IsReverse()):
...
def GetDistanceHistogramStatsScaleDuplicates(sam_lines, distance_limits, scale_by_num_occurances=True):
    ...
    while i < len(sorted_lines_by_distance):
        ...
        if (sam_line.IsMapped()):
            # if (sam_line.is_correct_ref_and_orient == 1):
            if (sam_line.is_correct_ref_and_orient == 1 and sam_line.min_distance <= sorted_distance_limits[
